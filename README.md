...
- 명시적으로 `--tickers`/`--universe-tickers`를 준 경우 custom user-watchlist universe 실행
- assistant briefing용 compact JSON / Markdown artifact 생성 (`output/assistant`)
...
uv run python -m screener.cli.main build-assistant-briefing --user-tickers TSLA,INFQ,PLTR,RKLB,GOOGL,NVDA --top-candidates 10
...
- `output/assistant/`: personal assistant용 compact briefing (`latest-user-briefing-screener.json`, `latest-user-briefing-screener.md`; custom basename 사용 시 별도 `{basename}.json/.md`)
...
src/screener/
  ...
  universe/           NASDAQ-100 universe loader + explicit custom ticker list parser
tests/               pytest suite
