...
- Phase 1 Priority 1 baseline is implemented: earnings context + relative strength vs QQQ
- Phase 2 Priority 2 baseline is implemented: ATR / volatility normalization
- Phase 3 Priority 3 baseline is implemented: candle structure / reversal bar quality
- Phase 3 refinement is implemented: upper wick / inside day / bullish engulfing-like signals
...
- `TWELVE_DATA_API_KEY` 또는 OpenClaw local secrets 파일(`~/.openclaw/secrets.json`, override: `SCREENER_OPENCLAW_SECRETS_PATH`)의 `/twelveData/apiKey` 는 불러오되, daily 기본 provider를 자동으로 바꾸지는 않습니다.
- `SCREENER_MARKET_DATA_PROVIDER=twelve-data` 또는 `yfinance` 로 명시하면 해당 provider를 사용합니다.
- Twelve Data API key는 `TWELVE_DATA_API_KEY` 환경변수가 OpenClaw secrets보다 우선합니다.
- Twelve Data는 free plan이 `8 credits/min` 이라, 전체 NASDAQ-100 daily screening의 기본 provider보다는 staged collection 용도로 다루는 편이 안전합니다.
...
## Future Extensions
- MACD 같은 추가 factor 반영
- ATR / volatility normalization refinement
- candle structure threshold tuning and follow-through validation
