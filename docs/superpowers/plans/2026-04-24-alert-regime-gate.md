...
**Goal:** QQQ가 20일 MA 아래이고 20일 수익률이 -5% 미만일 때 digest watchlist 후보를 상위 3개로 제한한다.

**Architecture:** `fetch_benchmark_context`에 MA 플래그를 추가하고, `policy.py`에 `evaluate_regime_gate`를 신설한 뒤, `builder.py`가 결과를 소비해 digest를 cap한다. `core.py`가 `benchmark_context`를 builder에 전달하는 연결고리 역할을 한다.
...
def test_evaluate_regime_gate_returns_pass_on_missing_data() -> None:
...
```bash
uv run pytest tests/test_alert_policy.py::test_evaluate_regime_gate_returns_pass_when_above_ma tests/test_alert_policy.py::test_evaluate_regime_gate_returns_capped_when_below_ma_and_return_below_threshold tests/test_alert_policy.py::test_evaluate_regime_gate_returns_pass_on_missing_data -v
```
...
`src/screener/alerts/schema.py`의 `AlertSummary` 클래스에 두 필드를 추가한다.
...
git commit -m "feat: add regime_gate fields to AlertSummary schema"
```
...
`tests/test_alert_builder.py`에 watchlist 전용 후보 생성 헬퍼와 테스트 2개를 추가한다.
...
from screener.scoring import BUY_REVIEW_TIER, WATCHLIST_TIER
```
...
`digest_members` 리스트를 완성한 뒤, `digest_state` 계산 전에 cap 로직을 삽입한다.
...
```python
    if regime.watchlist_cap is not None:
        ...
        digest_members = buy_review_members + watchlist_members[:regime.watchlist_cap]
...
```python
        summary=AlertSummary(
            ...
            suppressed_candidate_count=len(result.candidates) - len(next_tickers),
            quality_gate=quality_gate,
            regime_gate="capped" if regime.is_bearish else "pass",
...
`tests/test_pipeline.py` 끝에 아래 테스트를 추가한다. 이 테스트는 bearish QQQ 환경에서 파이프라인이 실행될 때 `alert-events.json`의 `summary.regime_gate`가 `"capped"`임을 검증한다.

```python
def test_pipeline_passes_benchmark_context_to_alert_builder(tmp_path: Path) -> None:
    import json

    # 25개 bar — 마지막 close(100)가 SMA-20(105) 아래, return_20d < -5%
    closes_above = [110.0] * 15   # 초반 15개
    closes_below = [100.0] * 10   # 마지막 10개 (SMA-20 = (110*10+100*10)/20 = 105, last=100 → below)
    all_closes = closes_above + closes_below

    from datetime import date as dt_date, timedelta
    rows = [
        {
            "date": dt_date(2026, 1, 1) + timedelta(days=i),
            "open": c, "high": c, "low": c, "close": c, "adj_close": c, "volume": 2e6,
        }
        for i, c in enumerate(all_closes)
    ]
    import pandas as pd
    qqq_df = pd.DataFrame(rows)
    ...
    provider = YFinanceMarketDataProvider(
        fetcher=StubFetcher(histories, failed_tickers={"NVDA": "no data"})
    )
    ...
    context = build_context(
        run_date=dt_date(2026, 1, 26),
        generated_at=datetime(2026, 1, 26, 20, 0, tzinfo=timezone.utc),
        output_dir=tmp_path,
    )

    pipeline.run(context)

    alert_path = tmp_path / "daily" / "latest" / "alert-events.json"
    payload = json.loads(alert_path.read_text(encoding="utf-8"))
    # regime_gate 필드가 존재하고 "pass" 또는 "capped"임을 확인 (값은 QQQ 데이터에 따라 결정됨)
    assert "regime_gate" in payload["summary"]
```
...
예상: `KeyError: 'regime_gate'` (summary에 필드 없음 — 아직 core.py가 benchmark_context를 전달하지 않아서)

- [ ] **Step 3: `core.py` 수정**

`src/screener/_pipeline/core.py`의 `build_daily_alert_document` 호출(line ~205)을 찾아 `benchmark_context` 파라미터를 추가한다.
...
(`benchmark_context`는 line ~117에서 이미 `dict[str, Any]`로 선언돼 있으므로 추가 변경 불필요)
...
- `uv run pytest` 전체 통과
- `output/daily/latest/alert-events.json`의 `summary` 필드에 `regime_gate`와 `regime_watchlist_cap` 포함
