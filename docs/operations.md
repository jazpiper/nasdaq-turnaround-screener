...
- `build-assistant-briefing`는 기본 `output/recommendations/recommendations.sqlite3`가 있으면 직전 recommendation run의 D+1 outcome을 `previous_top3_feedback`에 요약합니다. 데이터가 있으면 Markdown에 `Previous Top3 outcome feedback` 섹션이 추가되고, 직전 run은 있으나 D+1 outcome이 아직 settle/no-fill 되지 않았으면 unavailable reason만 노출합니다.
...
- settlement는 buy_limit_price가 horizon 구간 OHLC low에 닿으면 `filled`, 닿지 않으면 terminal `no_fill`로 기록합니다. 체결 후 horizon close return, SPY/QQQ 대비 return, max drawdown, target1/target2/stop/invalidation 첫 touch 날짜, target/stop same-candle ambiguous flag, time_exit_return_pct, R multiple, observation_note를 저장합니다. D+N calendar target이 주말/휴일이면 as-of-date 이내의 다음 관측 OHLC 일자로 settlement하고, 관측 가능한 horizon close가 아직 없거나 as-of-date가 horizon 전이면 `pending`과 `insufficient_future_prices`로 둡니다.
